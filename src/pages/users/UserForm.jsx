...
const UserForm = ({
  ...
  useEffect(() => {
    if (initialData) {
      reset({
        ...
        email: initialData.email || '',
        role: initialData.role || '',
        department: initialData.department || '', // यहाँ department API response जैसा हो, string
        password: '',
        confirmPassword: '',
      });
    }
  ...
  const departmentOptions = departments.map((dept) => ({
    value: dept.department_name, // id store karna best practice hai
    label: dept.department_name, // display text
  }));
  ...
  const onFormSubmit = (data) => {
    ...
    // onFormSubmit में
    const submitData = {
      id: initialData?.id || 0, // अगर edit है तो id भेजो
      username: data.username,
      email: data.email,
      password_hash: data.password,
      role: data.role,
      department: data.department, // यहाँ string ही भेजो
      created_at: initialData?.created_at || new Date().toISOString(),
    };

    // password confirm frontend ke liye tha, API ko nahi bhejna
    delete submitData.confirmPassword;

    // Agar edit mode me aur password empty hai
