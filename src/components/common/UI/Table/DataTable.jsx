...
const DataTable = ({
  ...
  // Defensive: pass (value, column, row), try-catch if custom render, fallback to '-'
  const renderCellValue = (value, column, row) => {
    if (column.render) {
      try {
        return column.render(value, row);
      } catch (e) {
        console.error('Error rendering column:', column.key, e);
        return '-';
      }
    }
            ...
            <AnimatePresence>
              {paginatedData.map((row, index) => (
                ...
                >
                  ...
                  {columns.map((column) => (
                    <td key={column.key}>{renderCellValue(row?.[column.key], column, row)}</td>
                  ))}
